...
**Track:** Healthcare / patient workflow (Vinmec · V-App ecosystem — optional anchor)  
**Product/app đã chọn:** Prototype **“Scan đơn → Lịch thuốc”** — không build lại app BV; slice gắn journey sau khám  
...
|---|---|---|---|
| Sau khám, user phải **đọc chữ viết tay / thuật ngữ** trên đơn và **tự gõ** giờ uống vào calendar hoặc app nhắc | _(ảnh đơn mẫu đã che thông tin cá nhân)_ | — (pain gốc) | Pain không phải “thiếu chatbot” mà **thiếu chuyển đơn → hành động** |
| Thử gõ thủ công 1 đơn 3 thuốc: ~10–15 phút, dễ sai `2 viên x 3 lần` → chỉ nhập 1 lần/ngày | _(ghi chú thời gian)_ | Failure (human) | Có ROI rõ cho automation **có xác nhận** |
| User không biết thuốc mới kê **là gì, uống lúc nào, tránh gì** — phải Google từng tên | _(search screenshot)_ | Happy (nếu có thẻ thuốc) | Cần **drug card** song song lịch, không chỉ OCR |
| V-AI (sáng): hỏi nhắc uống thuốc → hướng dẫn app **ngoài** (Long Châu), không deep-link Vinmec/V-App | `01-invidual-workshop/v-ai-app-teardown.md` | Failure | Analog: super-app chưa đóng loop **đơn → lịch in-app** |
| OCR thử trên 1 ảnh đơn in: nhận diện tên thuốc ~OK, **tần suất** (`sau ăn`, `3 lần/ngày`) cần sửa tay | _(ảnh OCR raw vs sửa)_ | Low-confidence / Correction | **Review screen bắt buộc** trước lưu lịch |
...
|---|---|---|---|
| App nhắc thuốc: user phàn nàn phải **nhập tay từng thuốc**, không scan đơn | App Store — category “medication reminder” (Medisafe, MyTherapy, …) _(điền link)_ | Bệnh nhân mãn tính / sau khám | Nhập tay = friction |
...
|---|---|---|---|
| **Medisafe / MyTherapy** | Nhắc lịch tốt; scan đơn thường trả phí / locale hạn | Lịch + push; scan là premium feature | ✅ Mock lịch, không build full |
| **Google Lens + manual** | OCR chữ, user tự copy | OCR ≠ schedule — thiếu parse tần suất | ✅ LLM parse JSON sau OCR |
| **V-AI (VinGroup)** | RAG Q&A, không parse đơn → calendar | Không đáp ứng job “từ đơn đến lịch” | ❌ Đổi track |
...
- V-AI / super-app chưa đóng loop đơn → hành động in-app.
...
- [x] Đổi user chính. _(từ user V-AI CSKH → người bệnh sau khám)_
...
...V-AI: handoff + deep-link cho hủy dịch vụ / trừ tiền.
