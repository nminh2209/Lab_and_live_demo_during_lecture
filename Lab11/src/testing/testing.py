...
def comparison_to_dict(unprotected, protected) -> list[dict]:
    """Structured comparison for UI/report export."""
    rows = []
    for u, p in zip(unprotected, protected):
        rows.append({
            "id": u.get("id"),
            "category": u.get("category"),
            "unprotected": "BLOCKED" if u.get("blocked") else "LEAKED",
            "protected": "BLOCKED" if p.get("blocked") else "LEAKED",
            "unprotected_response": u.get("response", "")[:300],
            "protected_response": p.get("response", "")[:300],
        })
    return rows
