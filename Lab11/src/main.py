...
async def part5_defense_pipeline():
    """Part 5: Assignment 11 defense-in-depth pipeline."""
    print("\n" + "=" * 60)
    print("PART 5: Defense Pipeline (Assignment 11)")
    print("=" * 60)

    from pipeline.defense_pipeline import run_assignment_test_suites

    results = await run_assignment_test_suites(use_llm=False)

    print("\n--- Test 1: Safe Queries (should PASS) ---")
    for r in results["safe_queries"]:
        status = "OK" if r["actual"] == "PASS" else "FAIL"
        _safe_print(f"  [{status}] {r['query'][:60]}")

    print("\n--- Test 2: Attacks (should be BLOCKED at input) ---")
    for r in results["attack_queries"]:
        status = "OK" if r["actual"] == "PASS" else "FAIL"
        pat = r.get("pattern") or "-"
        _safe_print(f"  [{status}] pattern={pat} | {r['query'][:50]}")

    print("\n--- Test 3: Rate Limiting (15 requests, max 10) ---")
    for r in results["rate_limit"]:
        print(f"  Request {r['request']:>2}: {r['status']}" + (
            f" (wait {r['wait_seconds']}s)" if r["wait_seconds"] else ""
        ))

    print("\n--- Test 4: Edge Cases ---")
    for r in results["edge_cases"]:
        blocked = "BLOCKED" if r["input_blocked"] else "ALLOWED"
        _safe_print(f"  [{blocked}] {r['query']}")

    demo = results["output_redaction_demo"]
    print("\n--- Output Redaction Demo (before vs after) ---")
    print(f"  Before: {demo.get('issues', [])}")
    print(f"  Sample redacted: {demo.get('redacted', '')[:100]}")
