...
Custom web demo (FastAPI + HTML, tiếng Việt có dấu, gợi ý câu hỏi mẫu):

```bash
uv sync --extra demo
uv run uvicorn demo.server:app --reload --port 8080
```

Open http://localhost:8080 — set `OPENAI_API_KEY` in `.env` first.

Legacy Streamlit UI (optional): `uv run streamlit run demo/app.py`
